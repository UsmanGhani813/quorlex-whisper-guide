...
export const Route = createFileRoute("/")({
  ...
  loader: async () => {
    const [services, projects, processSteps, industries] = await Promise.all([
      fetchServices(),
      fetchProjects(),
      fetchProcessSteps(),
      fetchIndustries(),
    ...
    return {
      ...
      nonCoreServices: services.filter((s) => !s.is_core),
...
function Home() {
  const { services, coreServices, nonCoreServices, featured, processSteps, industries } =
    Route.useLoaderData();
    ...
    <>
      <section className="relative overflow-hidden border-b border-border">
        ...
        <Container className="relative py-24 sm:py-32">
          <Eyebrow>Technology & software solutions · London, United Kingdom</Eyebrow>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground">
            Quorlex Soft is a technology and software solutions company building custom software,
            SaaS platforms, AI systems and cloud infrastructure for organisations worldwide.
          </p>
      ...
      <Section bordered={false}>
        ...
        {nonCoreServices.length > 0 ? (
          <p className="mt-6 text-sm text-muted-foreground">
            Plus {nonCoreServices.map((s) => s.name).join(", ")}.{" "}
            ...
            {featured.map((project) => (
              ...
              >
                <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-primary">
                  {project.industry?.name ?? project.market}
                </span>
      ...
      <Section className="bg-surface">
        <SectionHeading
          ...
          intro="A structured delivery process, so you always know what is happening and what comes next."
        />
