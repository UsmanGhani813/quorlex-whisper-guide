/**
 * Admin section builder — index + editor in ONE route.
 * Uses ?page=<slug> to switch between the list of pages and the editor for a
 * specific page. This avoids a fragile parent/child $param file, which some
...
const KINDS: {
  ...
  },
  {
    ...
    hint: "config = { member_slug, layout, show_publications, show_experience, show_degrees, cta }",
    defaults: {
      eyebrow: "Founder",
      title: "Meet the founder",
      config: {
        member_slug: "malik-aftab-hussain",
        layout: "detailed",
  ...
  },
  {
    ...
    hint: "Steps from the process_steps table.",
    defaults: {
      eyebrow: "How we ship",
  ...
  },
  {
    ...
    defaults: { config: { quote: "A quote.", attribution: "Someone" } },
  },
  {
    value: "rich_text",
    label: "Rich text",
  ...
  },
  {
    ...
    defaults: { title: "By the numbers", config: { metrics: [] as unknown[] } },
  },
  {
    value: "team_grid",
    label: "Team grid",
  ...
  },
  {
    value: "faq_accordion",
    label: "FAQ accordion",
    hint: "Renders every FAQ from the DB. Manage items in Admin → FAQ.",
    defaults: {
      eyebrow: "FAQ",
  ...
  },
  {
    value: "contact_form",
    label: "Contact form",
    hint: "config = { show_form: true|false }",
    defaults: {
      eyebrow: "Contact",
...
function SectionsIndex({ identity }: { identity: Identity }) {
    ...
    <AdminShell identity={identity}>
      ...
      <div className="mb-6 rounded-lg border border-border bg-card p-4 text-sm">
        ...
        <p className="mt-1 text-muted-foreground">
          Every public page is listed below. Add section blocks and they render at the top of that
          page.
        </p>
      </div>
      {loadError ? (
...
function SectionsEditor({ identity, pageSlug }: { identity: Identity; pageSlug: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        await reload((pageData as PageRow).id);
        setLoading(false);
      } catch (err) {
        if (!cancelled) {
          setLoadError(err instanceof Error ? err.message : String(err));
  ...
  return (
    <article
      className={`rounded-xl border ${
        s.is_enabled ? "border-border" : "border-dashed border-border/50 opacity-70"
      } bg-card p-5`}
    >
